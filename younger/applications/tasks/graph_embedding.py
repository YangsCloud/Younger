...
from younger.datasets.constructors.official.community_split import get_communities


def get_embedding_from_subgraph(parameters: tuple[pathlib.Path, pathlib.Path, pathlib.Path, pathlib.Path, dict[str, str]]):
    graph_path, subgraph_embedding_dir, save_dir, filtered_instances_dir, hash2embedding = parameters
    graph = load_pickle(graph_path)
    all_subgraph_with_labels = get_communities(graph)
    ...
    filtered_instance.load(filtered_instances_dir.joinpath(graph.graph['hash']))
    processed_embeddinhg = dict(
        model_id = filtered_instance.labels['model_name'],
...
def generate_graph_embeddings(subgraph_embedding_dir: pathlib.Path, graph_dir: pathlib.Path, save_dir: pathlib.Path, filtered_instances_dir: pathlib.Path, config_json: pathlib.Path, block_get_type: str, worker_number: int):
    with open(config_json, 'r') as f:
        hash2embedding = json.load(f)
    if block_get_type == 'greedy_modularity_communities':
        parameters = [(graph_path, subgraph_embedding_dir, save_dir, filtered_instances_dir, hash2embedding) for graph_path in graph_dir.iterdir()]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--filtered-instances-dir", type=str, help="The folder contains full instances whose filename is hash")
    ...
    filtered_instances_dir = pathlib.Path(args.filtered_instances_dir)
    ...
    generate_graph_embeddings(subgraph_embedding_dir, graph_dir, save_dir, filtered_instances_dir ,config_json, block_get_type, worker_number)