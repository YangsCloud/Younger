...
instance_dir = pathlib.Path('/Users/zrsion/instances-0.0.1')
save_dir = pathlib.Path('/Users/zrsion/instances_statistics')
...
def generater_statistics(instance_dir: pathlib.Path, save_dir: pathlib.Path,
    ...
    save_filename = "sta"
    if tasks != []:
        save_filename += "_" + "tasks" 
    if datasets != []:
        save_filename += "_" + "datasets" 
    if splits != []:
        save_filename += "_" + "splits"
    if metrics != []:
        save_filename += "_" + "metrics" 
    save_filename = save_filename.strip("_") + ".json"
                ...
                for label in labels:
                    ...
                    if combination in combinations:
                        ...
                        if combination not in final_dic:
                            final_dic[combination] = []
                        final_dic[combination].append(dic_to_add)
                        cnt += 1 # test
                        break
    
    with open(save_dir.joinpath(save_filename), 'w') as f:
